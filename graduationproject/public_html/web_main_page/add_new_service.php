...
if (isset($_POST['add_post'])){
    ...
    elseif (empty($servise)) {
        ...
        if ( $picture_name!=''){
                ...
                if( $picture_error==0){
                    if($picture_size<=30000000){
                        ...
                        if(move_uploaded_file($picture_tmp_name,$picture_dir)){
                            $insert=mysqli_query($connect,"INSERT INTO `post`( `titile`, `servise`, `start`, `close`, `location`, `image`, `author`, `servise_date`) 
                                                                           VALUES ('$title','$servise','$start' ,'$close','$location','$picture_DB_dir','$_SESSION[user_id]','$date')");
                            if (isset($insert)){
            ...
            $insert=mysqli_query($connect,"INSERT INTO `post`( `titile`, `servise`, `start`, `close`, `location`, `author`, `servise_date`)
                                                                           VALUES ('$title','$servise','$start','$close','$location','$_SESSION[user_id]','$date')");
            if (isset($insert)){
                $msg='<div class="alert alert-success" role="alert"> sucssesful added </div>';
                echo'<meta http-equiv="refresh" content="2;\'Services.php\'"/>';
            }
                    ...
                    <div class="card-body">
                        <form action="" method="post" enctype="multipart/form-data">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" id="title" name="title" value="<?php echo $title;?>" placeholder="Services title">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="post" class="col-sm-2 col-form-label">Services discribtion </label>
                                <div class="col-sm-10">
                                    <textarea class="form-control"id="full-featured-non-premium" name="servise" <?php echo $servise;?> rows="8"></textarea>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="Category" class="col-sm-2 col-form-label">start time</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="start" name="start" value="<?php echo $start;?>" placeholder="start time">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="Category" class="col-sm-2 col-form-label">close time</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="close" name="close" value="<?php echo $close;?>" placeholder="close time">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="Category" class="col-sm-2 col-form-label">location</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="location" name="location" value="<?php echo $location;?>" placeholder="location">
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10 ">
                                    <button type="submit" name="add_post" class="btn btn-danger">Add Services</button>
                                </div>
                            </div>
                        </form>
